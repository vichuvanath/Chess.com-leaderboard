...
const Leaderboard = () => {
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    if (!newUsername || !activeCommunity) return;
    try {
  ...
  return (
    <section id="leaderboard" className="leaderboard-section">
      ...
      <div className="leaderboard-container">
        ...
        {showAddUser && (
          <form onSubmit={handleAddUser} className="glass-panel admin-form">
            <input 
              type="text" 
        ...
        <div className="leaderboard-table-wrapper glass-panel">
          {loading ? (
            <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--accent-cyan)' }}>Synchronizing with Chess.com...</div>
          ) : (
              ...
              <tbody>
                {sortedPlayers.map((player, index) => (
                  <tr key={player.username}>
                    ...
                    <td>
                      <a href={`https://www.chess.com/member/${player.username}`} target="_blank" rel="noreferrer" className="player-cell" style={{ textDecoration: 'none', color: 'inherit' }}>
                        <img 
                          src={player.avatar} 
                          alt={player.username} 
                ...
                {sortedPlayers.length === 0 && (
                  <tr>
                    <td colSpan="4" style={{ textAlign: 'center', padding: '2rem' }}>No players in this community yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
